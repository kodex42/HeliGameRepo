...
Graph::Graph(int nodeWidth, int nodeHeight, GameObject nodeSprite, GameObject& ref) : nodeObj(nodeSprite), player(ref) {
...
Graph::Graph(std::vector<Node*>& nodes, GameObject nodeSprite, GameObject& ref) : nodeObj(nodeSprite), player(ref) {
	nodeMap = std::map<int, Node*>();
...
void Graph::update() {
	...
	//set the obstacle selected nodes, if node exists and is not the end-node or the current node.
	/*if (n != -1 && n != endNodeId && n != startNodeId) {
		Node& currentNode = *nodeMap.at(n);
	...
	for (int j = 0; j < nodeVec.size(); j++) {
		...
		for (int i = 0; i < nodeVec.at(j)->size(); i++) {
			...
			if (currentNode.getId() == startNodeId) {
				...
				glUniform3f(color_loc, 0.4f, -0.6f, 1.0f); // Pink for walls
			}
			else if (currentNode.isVisited()) {
				glUniform3f(color_loc, 1.0f, 0.0f, 1.0f); //If visited, be purple
			}
...
glm::vec3 Graph::getFirstLocation() {
	Node currentNode = *(nodeMap.at(getStartId()));
	return((glm::vec3(currentNode.getX(), currentNode.getY(), 0.0f)));
}

//The home-made way of getting the most efficient path.
//Takes everything around it and decides if it is on the path or not.
void Graph::getNext(){
	if (nodeVec.size() == 0) {	//if there are no nodes, do nothing.
		return;
	}
	QNode startNode = { &getNode(startNodeId), 0 };
	vector<Edge> neighbours = startNode.node->getEdges();
	for (int i = 0; i < neighbours.size(); i++) {
		Node* n = &getNode(startNode.node->getOtherNode(neighbours.at(i)).getId());
		if (n->isOnPath()) {
			startNodeId = n->getId();
			return;
		}
		else if (n->getId() == getEndId()) {
			startNodeId = getEndId();
			endNodeId = -1;
		}
	}
}

Node Graph::getNextId() {
	QNode startNode = { &getNode(startNodeId), 0 };
	vector<Edge> neighbours = startNode.node->getEdges();
	for (int i = 0; i < neighbours.size(); i++) {
		Node* n = &getNode(startNode.node->getOtherNode(neighbours.at(i)).getId());
...
std::vector<Node*> Graph::pathfind() {
	...
	for (int i = 0; i < nodeMap.size(); i++) {
		Node killNode = *nodeMap.at(i);
		killNode.unvisit();
	}

	//-1 means you are at your destination
	if (endNodeId != -1) {
		for (int i = 0; i < nodeVec.size(); i++) {
			for (int j = 0; j < nodeVec.at(i)->size(); j++) {
		...
		while (!pq.empty()) {
			...
			for (int i = 0; i < neighbours.size(); i++) {
				...
				//Adds the distance times the standard node weight to the calculation for determining where the object should travel
				int dist = abs(n->getX() - getNode(endNodeId).getX()) + abs(n->getY() - getNode(endNodeId).getY());
				int nodeCost = lowest.cost + neighbours.at(i).cost + dist * 10;
